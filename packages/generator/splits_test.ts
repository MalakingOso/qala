...
import { advancedFrequencyAlternatives, availableExercises, checkFrequency, chooseSplit, frequencyBand, mainLiftFrequency, slotsForDay } from "./splits.ts";
...
Deno.test("main-lift frequency meets the beginner band on every split", () => {
  ...
  for (const days of [2, 3, 4, 5, 6]) {
    ...
    assert(checkFrequency(chooseSplit(days), "beginner").length === 0, `${days}d clean for beginners`);
  }
});
...
Deno.test("main-lift frequency meets the intermediate band on every split", () => {
  ...
  for (const days of [2, 3, 4, 5, 6]) {
    assert(checkFrequency(chooseSplit(days), "intermediate").length === 0, `${days}d clean`);
  }
});
...
Deno.test("advanced alternatives: high 3-5x lower volume, low 1-2x higher volume", () => {
  ...
  assert(high.perSessionVolume.includes("lower"), "high alternative lowers per-session volume");
  assert(low.frequency.includes("1-2x"), "low alternative is 1-2x");
  assert(low.perSessionVolume.includes("higher"), "low alternative raises per-session volume");
  assert(frequencyBand("advanced", "high").lo === 3, "high band lo 3");
  assert(frequencyBand("advanced", "low").hi === 2, "low band hi 2");
  assert(frequencyBand("advanced", "default").hi === 4, "advanced default matches intermediate");
});
...
Deno.test("slots honour equipment and exclusions", () => {
  const noBarbell = availableExercises(["dumbbell", "bench", "cable", "machine", "pullupBar"], []);
  assert(!noBarbell.some((e) => e.equipment.includes("barbell")), "no barbell lifts without a bar");
  ...
  assert(availableExercises([], []).length === EXERCISE_CATALOG.length, "empty equipment means full gym");
  const day = chooseSplit(4).days[0];
  const slots = slotsForDay(day, "strength", noBarbell);
  assert(slots.every((s) => noBarbell.some((e) => e.id === s.exerciseId)), "slots use available lifts only");
});
