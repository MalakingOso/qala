...
Deno.test("weekly distance respects the two-week 30% cap, not the 10% rule", () => {
  ...
  assert(
    dists[1] > dists[0] * 1.1,
    "growth above 10% is allowed (no 10% rule)",
  );
});
...
Deno.test("long run never exceeds 1.10x the previous 30-day longest", () => {
  ...
  for (const week of plan.weeks) {
    ...
    assert(
      long.workout.targetKm <= cap + 1e-9,
      `week ${week.week} long run within 1.10x cap`,
    );
...
Deno.test("easy running stays at or above 80% of time", () => {
  ...
  for (const week of plan.weeks) {
    if (week.taperWeek) continue;
    assert(
      easyTimeShare(week) >= 0.8 - 1e-9,
      `week ${week.week} easy share >= 80%`,
    );
  }
});
...
Deno.test("quality count: one at 3 runs/wk, two at 4+", () => {
  ...
  const isQuality = (t: string) =>
    t === "tempo" || t === "intervals" || t === "surges";
...
Deno.test("race taper cuts 41-60% over two weeks and keeps intensity", () => {
  ...
  assert(
    weeks[weeks.length - 1].taperWeek && weeks[weeks.length - 2].taperWeek,
    "final two weeks taper",
  );
  const trainingKm = weeks.map((w) =>
    w.days.reduce((s, d) => s + d.workout.targetKm, 0)
  );
  const peak = Math.max(...trainingKm.slice(0, weeks.length - 2));
  const firstTaper = trainingKm[weeks.length - 2] /
    trainingKm[weeks.length - 3];
  assert(
    firstTaper >= 0.4 - 1e-9 && firstTaper <= 0.6 + 1e-9,
    "first taper week cuts 41-60%",
  );
  ...
  assert(
    support <= peak * 0.6 + 1e-9,
    "race-week support running stays tapered",
  );
  assert(
    raceWeek.days.every((d) => d.workout.type === "race" || !d.workout.hard),
    "only the race is hard in race week",
  );
  const preTaper = weeks[weeks.length - 3];
  assert(
    preTaper.days.some((d) => d.workout.hard),
    "intensity kept into the taper",
  );
  assert(
    weeks[weeks.length - 2].days.some((d) => d.workout.hard),
    "intensity kept during the taper",
  );
...
Deno.test("easy pace sits below 0.78 CS, threshold falls back to Riegel", () => {
  ...
  const noCS = thresholdPaceMps({
    ...BASE,
    criticalSpeedMps: undefined,
    recentRace: { distanceM: 5000, seconds: 1200 },
  });
  assert(
    noCS !== undefined && noCS < 5000 / 1200,
    "Riegel 10 km equivalent is slower than 5 km pace",
  );
  const plan = generateRunPlan(BASE);
  const easy = plan.weeks[0].days.find((d) => d.workout.type === "easy")!;
  assert(
    easy.workout.steps[0].paceZone!.includes("0.78 CS"),
    "easy zone names the 0.78 CS ceiling",
  );
});
