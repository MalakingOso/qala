...
import { daysBetween, decayValue, applyInput, muscleTau } from "./decay.ts";
...
Deno.test("daysBetween across US DST spring forward (Mar 8 2026)", () => {
  ...
  const dt = daysBetween("2026-03-07T12:00:00-05:00", "2026-03-09T12:00:00-04:00");
  assertClose(dt, 47 / 24, 1e-9, "DST spring dt");
});

Deno.test("daysBetween across DST fall back", () => {
  const dt = daysBetween("2026-10-31T12:00:00-04:00", "2026-11-02T12:00:00-05:00");
  assertClose(dt, 49 / 24, 1e-9, "DST fall dt");
});

Deno.test("daysBetween across month boundary", () => {
  assertClose(daysBetween("2026-01-31T00:00:00Z", "2026-02-02T00:00:00Z"), 2, 1e-9, "month");
  assertClose(daysBetween("2026-02-28T00:00:00Z", "2026-03-01T00:00:00Z"), 1, 1e-9, "feb-mar");
});
