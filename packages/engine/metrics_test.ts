...
Deno.test("intensity zones NL85/T85/NL/ARI", () => {
  ...
  assert(
    z.byZone["70-79.9"] === 5 && z.byZone["85-89.9"] === 3 &&
      z.byZone["90+"] === 2,
    "buckets",
  );
});
...
Deno.test("VL_m target/synergist weighting", () => {
  const vl = volumeLoad([{
    load: 100,
    reps: 10,
    muscles: [{ muscle: "quads", w: 1 }, { muscle: "hams", w: 0.5 }],
  }]);
...
Deno.test("PRs: rep at load, load at reps; >10 reps never e1RM", () => {
  ...
  assert(
    !prs.some((p) => p.kind === "e1rm" && p.reps === 12),
    "no e1RM over 10 reps",
  );
...
Deno.test("readiness: prs flag, sore block, low line behaviour", () => {
  ...
  const sore = predictReadiness(s, { prs: 8, soreness: { quads: 4 } }, [
    "quads",
  ]);
  ...
  s.prsHistory = [7, 7, 7, 8, 7, 8, 7].map((v, i) => ({
