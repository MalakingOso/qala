...
Deno.test("failure replaces effort", () => {
  ...
  assert(
    f.seconds === 240 && f.reasons.includes(ReasonCode.REST_FAILURE),
    `fail: ${f.seconds}`,
  );
  assert(!f.reasons.includes(ReasonCode.REST_EFFORT_HIGH), "no effort stacked");
  const fi = computeRest({
    approach: "strength",
    cls: "isolation",
    failed: true,
  });
  ...
  assert(
    e.seconds === 210 && e.reasons.includes(ReasonCode.REST_EFFORT_HIGH),
    `effort: ${e.seconds}`,
  );
  const el = computeRest({
    approach: "strength",
    cls: "main",
    rpeLogged: 7,
    rpeTarget: 8,
  ...
  assert(
    el.seconds === 150 && el.reasons.includes(ReasonCode.REST_EFFORT_LOW),
    `easy: ${el.seconds}`,
  );
});
...
Deno.test("short reps, late sets, drift, readiness each +30", () => {
  assert(
    computeRest({ approach: "hypertrophy", cls: "secondary", repsShort: true })
      .seconds === 150,
    "short",
  );
  assert(
    computeRest({ approach: "strength", cls: "main", setIndex: 4 }).seconds ===
      210,
    "late set",
  );
  assert(
    computeRest({ approach: "strength", cls: "main", setIndex: 2 }).seconds ===
      180,
    "early set none",
  );
  assert(
    computeRest({ approach: "hypertrophy", cls: "secondary", drift: true })
      .seconds === 150,
    "drift",
  );
  assert(
  ...
  assert(
    computeRest({ approach: "strength", cls: "main", setIndex: 5, drift: true })
      .seconds === 210,
    "no double",
  );
});
...
Deno.test("superset override 90 s, 60 s over budget", () => {
  ...
  assert(
    s.seconds === 90 && s.reasons.includes(ReasonCode.REST_SUPERSET),
    "pair 90",
  );
  const t = computeRest({
    approach: "hypertrophy",
...
Deno.test("early-start warning only below min on strength main", () => {
  ...
  assert(
    !earlyStartWarning("hypertrophy", "main", 150, 60),
    "non-strength silent",
  );
});
...
Deno.test("m after 10 on-target early taps at 90 s with B 120 is ~0.837", () => {
  ...
  for (let i = 0; i < 10; i++) {
    const r = learnRestMultiplier(cur, {
      m: cur.m,
      B: 120,
      adjustments: 0,
      actual: 90,
...
Deno.test("early miss never lowers m; warmup never updates", () => {
  ...
  const miss = learnRestMultiplier(before, {
    m: 0.9,
    B: 120,
    adjustments: 0,
    actual: 60,
  ...
  const full = learnRestMultiplier(before, {
    m: 0.9,
    B: 120,
    adjustments: 0,
    actual: 120,
  ...
  const wu = learnRestMultiplier(before, {
    m: 0.9,
    B: 120,
    adjustments: 0,
    actual: 60,
