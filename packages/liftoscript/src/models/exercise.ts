// Vendored from liftosaur (AGPL-3.0) `src/models/exercise.ts`.
// Qala adaptation: app-shell custom-exercise mutators that need the redux
// store (dispatch/lens-shmens/state) and program rename/edit thunks are omitted;
// the exercise DB, metadata and all pure helpers are unchanged.
import { ObjectUtils_keys, ObjectUtils_values, ObjectUtils_filter } from "../utils/object.ts";
import { Weight_build, Weight_gt, Weight_multiply, Weight_roundConvertTo, Weight_convertTo } from "./weight.ts";
import {
  type IExerciseId,
  type IEquipment,
  type IBarKey,
...
import { CollectionUtils_compact, CollectionUtils_sort, CollectionUtils_flat } from "../utils/collection.ts";
import { ExerciseImageUtils_exists } from "./exerciseImage.ts";
import { Equipment_getUnitOrDefaultForExerciseType, Equipment_currentEquipment } from "./equipment.ts";
