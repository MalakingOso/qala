import type { IPlannerProgram, ISettings, IDayData, IPlannerProgramDay } from "../../types.ts";
import { parser as plannerExerciseParser } from "./plannerExerciseParser.ts";
import memoize from "../../utils/memoize.ts";
import {
  type IPlannerEvalFullResult,
  type IPlannerEvalResult,
  type IPlannerSyntaxPointer,
  PlannerExerciseEvaluator,
  PlannerSyntaxError,
} from "./plannerExerciseEvaluator.ts";
import type {
...
import { PlannerKey_fromFullName, PlannerKey_fromPlannerExercise } from "./plannerKey.tsx";
import { ObjectUtils_isEqual, ObjectUtils_clone, ObjectUtils_keys, ObjectUtils_values } from "../../utils/object.ts";
import { Weight_type } from "../../models/weight.ts";
import { PlannerProgram_fullToWeekEvalResult, PlannerProgram_generateFullText } from "./models/plannerProgram.ts";
import { ScriptRunner } from "../../parser.ts";
import { Progress_createEmptyScriptBindings, Progress_createScriptFunctions } from "../../models/progress.ts";
import { LiftoscriptSyntaxError } from "../../liftoscriptEvaluator.ts";
import type { IPlannerReuseSection } from "../../syntaxErrorTypes.ts";
import {
...
} from "./models/plannerProgramExercise.ts";
...
export function PlannerEvaluator_getFirstError(evaluatedWeeks: IPlannerEvalResult[][]): PlannerSyntaxError | undefined {
...
export function PlannerEvaluator_evaluateDay(
...
export function PlannerEvaluator_getPerDayEvaluatedWeeks(
...
export function PlannerEvaluator_changeExerciseName(
  text: string,
  from: string,
  to: string,
  settings: ISettings
...
export function PlannerEvaluator_getFullEvaluatedWeeks(
...
export function PlannerEvaluator_getDayIndexFromWeekAndDayInWeekIndex(
  evaluatedWeeks: IPlannerEvalResult[][],
  weekIndex: number,
...
export function PlannerEvaluator_fillRepeats(
  exercise: IPlannerProgramExercise,
  evaluatedWeeks: IPlannerEvalResult[][],
  dayInWeekIndex: number,
  byExerciseWeekDay: IByExerciseWeekDay<IPlannerProgramExercise>
...
export function PlannerEvaluator_fillSetReuses(
  ...
  if (exercise.reuse && exercise.points.reuseSetPoint) {
    ...
    const originalExercises = PlannerEvaluator_findOriginalExercisesAtWeekDay(
      ...
      reuse.fullName,
      evaluatedWeeks,
      // Qala: trailing `?? 1` dropped, `weekIndex + 1` is never nullish; semantics unchanged.
...
export function PlannerEvaluator_fillEvaluatedSetVariations(exercise: IPlannerProgramExercise): void {
...
export function PlannerEvaluator_fillDescriptions(
  exercise: IPlannerProgramExercise,
  evaluatedWeeks: IPlannerEvalResult[][],
  weekIndex: number,
  dayIndex: number
...
export function PlannerEvaluator_fillProgressReuses(
  evaluatedWeeks: IPlannerEvalResult[][],
  exercise: IPlannerProgramExercise,
  settings: ISettings,
  metadata: IPlannerEvalMetadata
...
export function PlannerEvaluator_checkUpdateScript(
  exercise: IPlannerProgramExercise,
  settings: ISettings,
  dayData: IDayData
): void {
...
export function PlannerEvaluator_fillUpdateReuses(
  evaluatedWeeks: IPlannerEvalResult[][],
  exercise: IPlannerProgramExercise,
  settings: ISettings,
  metadata: IPlannerEvalMetadata
...
export function PlannerEvaluator_checkUnknownExercises(
  exercise: IPlannerProgramExercise,
  metadata: IPlannerEvalMetadata
): void {
  if (metadata.notused.has(exercise.key)) {
...
export function PlannerEvaluator_findReusedDescriptions(
  reusingName: string,
...
export function PlannerEvaluator_findOriginalExercisesAtWeekDay(
  settings: ISettings,
  fullName: string,
...
export function PlannerEvaluator_evaluateFull(
  fullProgramText: string,
  settings: ISettings
...
export function PlannerEvaluator_findLastWeekExercise(
  program: IPlannerEvalResult[][],
  weekIndex: number,
  dayIndex: number,
  exercise: IPlannerProgramExercise,
...
export function PlannerEvaluator_setByExerciseWeekDay<T, U extends Record<string, Record<number, Record<number, T>>>>(
  coll: U,
  exercise: string,
  weekIndex: number,
  dayIndex: number,
...
export function PlannerEvaluator_setByWeekDayExercise<T, U extends Record<number, Record<number, Record<string, T>>>>(
  coll: U,
  exercise: string,
  weekIndex: number,
  dayIndex: number,
...
export function PlannerEvaluator_iterateOverExercises(
  program: IPlannerEvalResult[][],
  cb: (
    weekIndex: number,
    dayInWeekIndex: number,
