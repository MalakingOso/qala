// Vendored from liftosaur (AGPL-3.0) `src/liftoscriptFns.ts`.
// Qala addition: numeric engine bindings (readiness/prs/soreness/
// fatigueLocal/deload/recWeightPct/recSets) on VScriptBindings, PLAN section 5.
import * as v from "valibot";
import { VWeight, VPercentage } from "./types.ts";
import type { IScriptFnContext } from "./models/progress.ts";
...
export const VScriptBindings = v.object({
  ...
  setIndex: v.number(),
  // Qala engine bindings (PLAN section 5). Plain numbers so the evaluator's
  // type system is untouched. Defaults are neutral: full readiness, no
  // soreness, no fatigue, no deload, no recommended change.
  readiness: v.optional(v.number(), 1),
  prs: v.optional(v.number(), 0),
  soreness: v.optional(v.number(), 1),
  fatigueLocal: v.optional(v.number(), 0),
  deload: v.optional(v.picklist([0, 1, 2]), 0),
  recWeightPct: v.optional(v.number(), 0),
  recSets: v.optional(v.number(), 0),
});
