...
| Shells | One codebase, two distinct shells. Desktop = author (editor + live preview, exercise DB, coach memory, graphs, calibration). Phone = logger and run recorder (check-in, set table with big steppers, RPE, rest timer, GPS run recording, guided runs, read-only program and run plan). No authoring on the phone. |
...
## 9. Milestone M5: desktop shell (author)

Panels: program list; CodeMirror 6 editor with Lezer highlighting for both grammars and inline parse errors; live evaluated preview beside it (weeks, days, sets, computed weights, reps at 85%+, sets per muscle, time including warm-ups); exercise DB table (seed + custom, hide/edit, muscle tags required on custom entries, per-exercise bar and rest-class overrides); coach-memory editor with accept/reject for Gemma-proposed facts; history table; graphs; calibration view (per-lift `p0, k1, theta`, residuals, observation count). Screen spec: `DESIGN.md` 7.15-7.16.
