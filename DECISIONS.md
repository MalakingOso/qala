...
| L8 | The 2026-09-13 visual refinement keeps its layout, chart and control fixes (responsive charts drawn in CSS pixels, desktop sidebar and toolbar, recomposed Today and Plan, themed form controls, quieter chart headers), but its surface treatment is reverted: back to 2px borders, hard offset shadows with zero blur, 4/6/8px radii, flat-at-rest cards that lift on hover, Beamer's input and active-nav treatment, and no logo tile. | decided (owner: the refinement "did make the app seem more like a sterile AI app instead of having the DNA of coming from Beamer", while the charting and button fixes are to be kept) | Keeping the 1px / soft-shadow / 6-12px-radius look. | DESIGN 1, 2, 5; `apps/web/src/theme/tokens.css` |
...
| U11 | Desktop is stats-first, not author-only: an Overview dashboard is the desktop's home; the sidebar splits into a Stats group (Overview, Lifts, Running, Body, History) and an Author group (Programs, Exercises, Coach memory, Calibration) below it; lift, session, run and muscle drill-down pages link out of the stats views; session notes and dismissing a flagged PR are the desktop's one write surface, otherwise read-only. | decided (2026-09-13; owner: "It shouldn't just be this other coaching thing... It should be more well-formatted, easier to click through. Right now it's hideous") | PLAN 9's "desktop shell (author)" framing; DESIGN 7.15's flat sidebar list (Today, Plan, Programs, Run plans, Exercises, Body, Progress, Coach, Settings), which the shipped v1 build had already diverged from. | `apps/web/src/shell-desktop/`, this file |
...
| U13 | "This week" is a seven-day strip, not stacked columns: one bar per day sized by the day's load and filled by the part done, in the rail's vocabulary (done teal, today ember and named, later outlined, rest a dash), a lift/run/both/rest glyph under each, a caption for the tapped or hovered day and a week total. Same `WeekLoadDay` data and table view. | decided (owner: the stacked chart "doesn't make any sense. It's very hard to understand and to follow"; the old encoding also stacked planned load on top of done load, so a finished day drew twice its size) | Stacked columns splitting lifting and running with planned outlined above done; removing the chart from Today. | DESIGN 6.3, 7.1; `apps/web/src/shared/charts/WeeklyLoad.tsx` |
...
| Desktop rework | 2026-09-13 | The shipped v1 desktop shell, called "hideous" | Overview dashboard as home; Stats/Author sidebar grouping; lift, session, run and muscle drill-downs; session notes and PR-flag dismissal as the desktop's editable surface (U11). |
| Visual refinement and revert | 2026-09-13 | A smoothing pass by another agent (`docs/visual-refinement.md`), then its result | Owner: keep its chart, button and layout fixes; it reads as "a sterile AI app", so the Beamer surface rules come back (L8); the stacked week-load chart is replaced by a week strip (U13). |
