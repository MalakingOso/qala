...
const TRAINED = [
  { muscle: "quads", last: "trained Thu", meaning: ["never sore", "healed well before", "healed just in time", "still sore now"] },
  { muscle: "glutes", last: "trained Thu", meaning: ["never sore", "healed well before", "healed just in time", "still sore now"] },
  { muscle: "hamstrings", last: "trained Thu", meaning: ["never sore", "healed well before", "healed just in time", "still sore now"] },
  { muscle: "calves", last: "trained Tue", meaning: ["never sore", "healed well before", "healed just in time", "still sore now"] },
];

export function CheckinPage({ restDay = false }: { restDay?: boolean }) {
  const { stages, settings, queueOp } = useQala();
  const isRest = restDay || stages.length === 3;
  ...
  const tipMuscles = soreList.length > 0 ? soreList.map(([m]) => m).join(" + ") : "hips";
  const tool = settings.equipment.percussion && !soreList.length ? "Theragun" : "foam roller";
  ...
  return (
    <div>
      ...
      <Card hero>
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }} role="radiogroup" aria-label="Recovery">
          {Array.from({ length: 11 }, (_, v) => (
            <button
              ...
              onClick={() => setPrs(v)}
              className="icon-btn"
              style={prs === v ? { background: "var(--accent)", color: "var(--on-accent)" } : undefined}
            >
              {v}
            </button>
          ))}
        </div>
        <p className="kbd-hint">0-2 worse session expected · 3-7 normal · 8-10 better</p>
        <p className="group-label">Soreness 1-4</p>
        {TRAINED.map((t) => (
          <div key={t.muscle} style={{ marginBottom: 8 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <strong style={{ textTransform: "capitalize" }}>{t.muscle}</strong>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[1, 2, 3, 4].map((v) => (
                <button
                  ...
                  className="icon-btn"
                  title={t.meaning[v - 1]}
                  style={(sore[t.muscle] ?? 0) === v ? { background: "var(--bg-active)", outline: "2px solid var(--accent)" } : undefined}
                >
                  {v}
                </button>
              ))}
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
          {chips.map((c) => (
            <Chip key={c} onRemove={() => setChips((cs) => cs.filter((x) => x !== c))}>
              {c}
            </Chip>
          ))}
        </div>
