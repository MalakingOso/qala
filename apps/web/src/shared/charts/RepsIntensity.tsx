...
export function RepsIntensity({
  ...
  const x = useMemo(() => scaleLinear<number>({ domain: [0, total], range: [8, W - 8] }), [total]);
  let acc = 0;
    ...
    >
      ...
      <svg viewBox={`0 0 ${W} 64`} width="100%" role="presentation">
        {zones.map((z, i) => {
          ...
          acc += z.reps;
          return (
            <g key={z.label} tabIndex={0} role="img" aria-label={`${z.label}: ${z.reps} reps`}>
              <title>{`${z.label}: ${z.reps}`}</title>
              <rect x={x0} y={8} width={Math.max(0, x1 - x0)} height={22} fill={ORDINAL[i % ORDINAL.length]} />
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="presentation">
        {lifts.map((l, i) => {
          const yy = 10 + i * 44;
          return (
            <g key={l.lift} tabIndex={0} role="img" aria-label={`${l.lift}: ${l.actual} of target ${l.target}`}>
              <title>{`${l.lift}: ${l.actual} / ${l.target}`}</title>
              <text x={112} y={yy + 18} fontSize={12} textAnchor="end" fill="var(--fg)">
                {l.lift}
              </text>
              <rect x={120} y={yy} width={Math.max(0, x(l.actual) - 120)} height={22} fill="var(--viz-2)" rx={2} />
              <line x1={x(l.target)} x2={x(l.target)} y1={yy - 4} y2={yy + 26} stroke="var(--accent)" strokeWidth={3} />
            </g>
          );
        })}
      </svg>
