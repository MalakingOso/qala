...
export function FatigueByMuscle({
  ...
  muscles: { muscle: string; lifting: number; running: number; ready: string }[];
}) {
  ...
  const rows = muscles.map((m) => [m.muscle, m.lifting.toFixed(1), m.running.toFixed(1), m.ready]);
  return (
    <ChartShell
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="presentation">
        {muscles.map((m) => {
          ...
          return (
            <g key={m.muscle} tabIndex={0} role="img" aria-label={`${m.muscle}: lifting ${m.lifting}, running ${m.running}, ready ${m.ready}`}>
              <title>{`${m.muscle}: ready ${m.ready}`}</title>
              <text x={112} y={yy + bh + 2} fontSize={11} textAnchor="end" fill="var(--fg)">
                {m.muscle}
              </text>
              <Bar x={120} y={yy} width={lw} height={bh} fill={CATEGORICAL[0]} rx={0} />
              <Bar x={120 + lw} y={yy} width={rw} height={bh} fill={CATEGORICAL[1]} rx={0} />
              <text x={W - 12} y={yy + bh + 2} fontSize={10} textAnchor="end" fill="var(--fg-muted)">
                {m.ready}
              </text>
            </g>
          );
        ...
        <g fontSize={11} fill="var(--fg)">
          <rect x={120} y={H - 16} width={10} height={10} fill={CATEGORICAL[0]} />
          <text x={134} y={H - 7}>lifting</text>
          <rect x={190} y={H - 16} width={10} height={10} fill={CATEGORICAL[1]} />
...
export function RunSpark({ points, label }: { points: number[]; label: string }) {
  const H = 64;
  const x = useMemo(
    () =>
      scalePoint<number>({
  ...
  const y = useMemo(() => {
    ...
    return scaleLinear<number>({ domain: [lo - pad, hi + pad], range: [H - 6, 6] });
  }, [points]);
  return (
    <ChartShell
