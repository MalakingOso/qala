...
export function WeeklyLoad({ days }: { days: WeekLoadDay[] }) {
  const { x, y } = useMemo(() => {
    const max = Math.max(1, ...days.map((d) => d.liftDone + d.runDone + d.liftPlanned + d.runPlanned));
    return {
      x: scaleBand<string>({
      ...
      y: scaleLinear<number>({ domain: [0, max], range: [H - PAD.bottom, PAD.top] }),
    };
  }, [days]);
  ...
  return (
    <ChartShell title="This week's load" head={["Day", "Done", "Planned"]} rows={rows} label={label}>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="presentation">
        {days.map((d, i) => {
          ...
          return (
            <Group key={i}>
              ...
              <Bar x={bx} y={y(0) - liftDoneH} width={bw} height={Math.max(0, liftDoneH)} fill={CATEGORICAL[0]} rx={0} />
              <Bar
                x={bx}
              ...
              />
              <Hit x={bx + bw / 2} y={H - PAD.bottom - 4} label={`${d.day}: done ${d.liftDone + d.runDone}, planned ${d.liftPlanned + d.runPlanned}`}>
                <circle r={0} fill="none" />
              </Hit>
              {d.today ? (
                <text x={bx + bw / 2} y={PAD.top - 8} fontSize={10} textAnchor="middle" fill="var(--accent)">
        ...
        <AxisBottom
          ...
          tickLabelProps={{ fontSize: 11, fill: "var(--fg)", textAnchor: "middle" }}
          hideAxisLine
          hideTicks
        />
        <g fontSize={11} fill="var(--fg)">
          <rect x={PAD.left} y={2} width={10} height={10} fill={CATEGORICAL[0]} />
          <text x={PAD.left + 14} y={11}>lifting</text>
          <rect x={PAD.left + 70} y={2} width={10} height={10} fill={CATEGORICAL[1]} />
