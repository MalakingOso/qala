...
export function SetsByMuscle({
  ...
  const { x, y } = useMemo(() => {
    ...
    return {
      x: scaleLinear<number>({ domain: [0, max], range: [PAD.left, W - PAD.right] }),
      y: scaleBand<string>({
  ...
  const rows = muscles.map((m) => [m.muscle, String(m.earlier), String(m.today)]);
  return (
    <ChartShell
      title={title}
      head={["Muscle", "Earlier", "Today"]}
      rows={rows}
      label={`${title}: ${muscles.map((m) => `${m.muscle} ${m.earlier + m.today}`).join(", ")}.`}
    >
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="presentation">
        {[10, 20].map((v) => (
          <line key={v} x1={x(v)} x2={x(v)} y1={PAD.top} y2={H - PAD.bottom} stroke="var(--border-strong)" strokeWidth={1} />
        ))}
        {muscles.map((m) => {
          ...
          return (
            <g key={m.muscle}>
              <text x={PAD.left - 8} y={yy + bh / 2 + 4} fontSize={11} textAnchor="end" fill="var(--fg)">
                {m.muscle}
              </text>
              <Bar x={PAD.left} y={yy} width={Math.max(0, e1 - PAD.left)} height={bh} fill="var(--mark-gray)" rx={0} />
              <Bar x={e1} y={yy} width={Math.max(0, t1 - e1)} height={bh} fill="var(--accent)" rx={0} />
              <Hit x={t1 + 14} y={yy + bh / 2} label={`${m.muscle}: ${m.earlier} earlier, ${m.today} today`}>
